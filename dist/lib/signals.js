...
import { HUB_URL, AGENT_ID } from "./config.js";
import { fetchPropertyFromHub, hubHeaders } from "./hub.js";
let signalWs = null;
let subscribedStations = [];
let pendingResolve = null;
const signalQueue = [];
const MAX_QUEUE_SIZE = 50;
export function getSubscribedStations() { return subscribedStations; }
export function setSubscribedStations(stations) { subscribedStations = stations; }
export function isWsOpen() { return !!signalWs && signalWs.readyState === WebSocket.OPEN; }
export function connectSignalWs() {
    const wsUrl = HUB_URL.replace(/^http/, "ws");
    signalWs = new WebSocket(wsUrl);
    signalWs.on("message", (raw) => {
        try {
    ...
    signalWs.on("close", () => {
        ...
        if (subscribedStations.length > 0)
            setTimeout(connectSignalWs, 3_000);
    });
    signalWs.on("error", () => { });
}
function formatSignalEvent(msg) {
...
export async function waitForSignal() {
    if (signalQueue.length > 0)
        return formatSignalEvent(signalQueue.shift());
...
export async function tryClaimPendingTask(station) {
    try {
