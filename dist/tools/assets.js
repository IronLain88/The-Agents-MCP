...
import { hubHeaders, fetchPropertyFromHub } from "../lib/hub.js";
import { findAsset } from "../lib/asset-lookup.js";
export function register(server) {
    server.tool("sync_property", "Refresh your local view of the property from the hub.", {}, async () => {
        try {
            const property = await fetchPropertyFromHub();
            return { content: [{ type: "text", text: `Property synced (${property.assets?.length || 0} assets)` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Sync failed: ${err}` }] };
        }
    });
    server.tool("list_assets", "List all assets on your property", {}, async () => {
        try {
            const property = await fetchPropertyFromHub();
            const assets = property.assets || [];
            if (assets.length === 0)
                return { content: [{ type: "text", text: "No assets on property" }] };
            ...
            return { content: [{ type: "text", text: `Failed to fetch property: ${err}` }] };
        }
    });
    server.tool("add_asset", "Add furniture to your property. Set 'station' to make it a place your agent can walk to. " +
        ...
        try {
            ...
            if (remote_station)
                body.remote_station = remote_station;
            if (openclaw_task)
                body.openclaw_task = true;
            if (archive)
                body.archive = true;
            if (welcome)
                body.welcome = true;
            ...
            if (!res.ok) {
                ...
                return { content: [{ type: "text", text: `Failed to add asset: ${err.error}` }] };
            }
            const { asset } = await res.json();
            const posStr = asset.position ? `at (${asset.position.x}, ${asset.position.y})` : "in inventory";
            return { content: [{ type: "text", text: `Added asset "${name}" (${asset.id}) ${posStr}` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to add asset: ${err}` }] };
        }
    });
    server.tool("remove_asset", "Remove an asset from your property", { asset_id: z.string().describe("ID of asset to remove") }, async ({ asset_id }) => {
        try {
            const res = await fetch(`${HUB_URL}/api/assets/${encodeURIComponent(asset_id)}`, { method: "DELETE", headers: hubHeaders() });
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: res.statusText }));
                return { content: [{ type: "text", text: `Failed to remove asset: ${err.error}` }] };
            }
            const { removed } = await res.json();
            return { content: [{ type: "text", text: `Removed asset "${removed.name || removed.id}"` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to remove asset: ${err}` }] };
        }
    });
    server.tool("move_asset", "Move an asset to a new position", {
        ...
        try {
            ...
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: res.statusText }));
                return { content: [{ type: "text", text: `Failed to move asset: ${err.error}` }] };
            }
            const { asset } = await res.json();
            return { content: [{ type: "text", text: `Moved "${asset.name || asset.id}" to (${x}, ${y})` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to move asset: ${err}` }] };
        }
    });
    server.tool("attach_content", "Attach content to an asset from a local file", {
        ...
        try {
            ...
            if (!resolved.startsWith(projectRoot + sep) && resolved !== projectRoot) {
                return { content: [{ type: "text", text: "Error: path must be within project directory" }] };
            }
            ...
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: res.statusText }));
                return { content: [{ type: "text", text: `Failed to attach: ${err.error}` }] };
            }
            const { asset } = await res.json();
            return { content: [{ type: "text", text: `Attached ${file_path} to "${asset.name || asset.id}"` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to attach content: ${err}` }] };
        }
    });
    server.tool("write_content", "Write markdown content directly to an asset's content section by name or station", {
        ...
        try {
            ...
            if (!asset)
                return { content: [{ type: "text", text: `Asset "${name}" not found. Use list_assets to see available assets.` }] };
            ...
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: res.statusText }));
                return { content: [{ type: "text", text: `Failed to write: ${err.error}` }] };
            }
            return { content: [{ type: "text", text: `Content written to "${asset.name || asset.station || asset.id}" (${content.length} chars)` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to write content: ${err}` }] };
        }
    });
    server.tool("read_asset_content", "Read content attached to an asset on your property by name or ID", { name: z.string().describe("Asset name or ID (fuzzy match)") }, async ({ name }) => {
        try {
            const property = await fetchPropertyFromHub();
            const asset = findAsset(property.assets || [], name);
            if (!asset)
                return { content: [{ type: "text", text: `Asset "${name}" not found. Use list_assets to see available assets.` }] };
            if (!asset.content)
                return { content: [{ type: "text", text: `Asset "${asset.name || asset.id}" has no content attached.` }] };
            const footer = asset.content.source ? `\n\n---\n*Source: ${asset.content.source}*` : "";
            return { content: [{ type: "text", text: `# ${asset.name || asset.id}\n\n${asset.content.data}${footer}` }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Failed to fetch property: ${err}` }] };
        }
