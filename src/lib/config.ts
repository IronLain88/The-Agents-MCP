...
export function getGroup(state: string): string {
  switch (state) {
...
try {
  ...
  if (!["http:", "https:"].includes(parsed.protocol)) throw new Error("HUB_URL must use http or https protocol");
  if (parsed.hostname !== "localhost" && parsed.hostname !== "127.0.0.1") {
    console.error(`[agent-visualizer] WARNING: HUB_URL points to non-localhost host: ${parsed.hostname}`);
  }
} catch (err) {
  if (err instanceof TypeError) { console.error(`[agent-visualizer] FATAL: Invalid HUB_URL: ${HUB_URL}`); process.exit(1); }
  if (err instanceof Error && err.message.startsWith("HUB_URL")) { console.error(`[agent-visualizer] FATAL: ${err.message}`); process.exit(1); }
}
