...
export function register(server: McpServer): void {
    ...
    async ({ name }) => {
      try {
        ...
        if (!name) {
          ...
          setSubscribedStations(taskStations.map((a: Asset) => a.station).filter((s): s is string => !!s));
          if (!isWsOpen()) connectSignalWs();
          const stations = getSubscribedStations();
          const pending = taskStations.filter((a: Asset) => {
  ...
  server.tool(
    ...
    async () => {
      const stations = getSubscribedStations();
      if (stations.length === 0) return { content: [{ type: "text" as const, text: "Not subscribed to any station. Call subscribe() first." }] };
      if (!isWsOpen()) connectSignalWs();

      for (const station of stations) {
        const pending = await tryClaimPendingTask(station);
        if (pending) return { content: [{ type: "text" as const, text: pending }] };
      }
